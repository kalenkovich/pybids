...
@pytest.fixture(scope="module")
def layout_ds117():
    data_dir = get_test_data_path() / 'ds000117'
    return BIDSLayout(data_dir)
...
def layout_ds005_derivs():
    ...
    deriv_dir = data_dir / 'derivatives' / 'events'
...
def layout_ds005_multi_derivs(request, db_dir):
    ...
    deriv_dir1 = get_test_data_path() / 'ds005_derivs'
    deriv_dir2 = data_dir / 'derivatives' / 'events'
...
def layout_synthetic(request, db_dir):
    path = get_test_data_path() / 'synthetic'
    database_path = str(db_dir / request.param) if request.param else None
